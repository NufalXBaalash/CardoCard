...
class _MedicalInfoPageState extends State<MedicalInfoPage>
  ...
  bool get _isDoctor {
    ...
    return r == 'doctor' || r.contains('doctor') || r.contains('طبيب') || r.contains('دكتور');
  }

  final _specialties = const [
    'Cardiologist', 'Neurology', 'Pediatrician', 'Orthopedics',
    'Dermatologist', 'Psychiatrist', 'Oncologist', 'Gastroenterologist',
    'General Surgery', 'ENT', 'Ophthalmology', 'Dentistry',
  ];
  ...
  Future<void> _submitMedicalInfo() async {
    ...
    if (_phoneController.text.trim().isEmpty) {
      setState(() => _phoneError = context.translate('phone_required'));
      isValid = false;
    }

    if (_addressController.text.trim().isEmpty) {
      setState(() => _addressError = context.translate('address_required'));
      isValid = false;
    }

    if (_isDoctor && (_selectedSpecialty == null || _selectedSpecialty!.isEmpty)) {
      setState(() => _authError = 'Please select your specialty');
      isValid = false;
    }
    ...
    try {
      ...
      final String normalizedRole = _isDoctor ? 'doctor' : 'patient';
      ...
      await SupabaseService.upsertProfile(userId, {
        ...
        if (widget.profileImageBase64 != null) ...{
          'profile_image_base64': widget.profileImageBase64,
          'profile_image_updated_at': DateTime.now().toUtc().toIso8601String(),
        },
      });

      // 3. Save medical info to Supabase (patients only)
      if (!_isDoctor) {
        await SupabaseService.upsertMedicalInfo(userId, {
          'blood_type': _bloodType,
          'has_diabetes': _hasDiabetes,
          'has_asthma': _hasAsthma,
          'medical_info_added_at': DateTime.now().toUtc().toIso8601String(),
        });
      }

      // 4. If doctor, create doctor record
      if (_isDoctor) {
        await SupabaseService.client.from('doctors').insert({
          'name': widget.fullName,
          'specialty': _selectedSpecialty ?? 'General',
          'specialty_key': (_selectedSpecialty ?? 'general').toLowerCase().replaceAll(' ', '_'),
      ...
      // Navigate to Iflogin which handles role-based routing
      if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_authError), backgroundColor: Colors.red),
      );
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error: $e'), backgroundColor: Colors.red, duration: Duration(seconds: 8)),
        );
      }
    }
  }
